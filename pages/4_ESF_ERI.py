...
<style>
    ...
    /* ── Dataframe: celdas con texto legible ── */
    ...
    /* Forzar fondo blanco en el contenedor del dataframe */
...
def cargar_extracto(archivo) -> tuple:
    ...
    if es_excel:
        ...
        for enc in ["latin1", "utf-8", "cp1252"]:
            try:
                df = pd.read_csv(
                    io.BytesIO(contenido), encoding=enc,
    ...
    if df is None or df.shape[1] < 9:
        st.error(f"El archivo no tiene suficientes columnas (se necesitan al menos 9). "
...
def cargar_excel(archivo) -> pd.DataFrame:
    ...
    col_debito  = next((c for c in df.columns if "d" in c.lower() and "bito" in c.lower()), None)
    col_credito = next((c for c in df.columns if "cr" in c.lower() and "dito" in c.lower()), None)

    if not col_debito or not col_credito:
        st.error(f"No se encontraron columnas Débito/Crédito. Columnas: {list(df.columns)}")
        return None

    col_tercero = next((c for c in df.columns if "tercero" in c.lower()), None)
...
def comparar(serie_csv, desc_csv, df_excel, col_debito, col_credito, col_tercero, tolerancia):
    map_debito  = {}
    map_credito = {}
    if col_tercero:
...
<div class="header-block">
    ...
    <p>Verifica si los valores de la columna I del extracto bancario (CSV)
       están registrados en el libro contable (Excel)</p>
</div>
""", unsafe_allow_html=True)
...
with col1:
    ...
    archivo_csv = st.file_uploader("Subir extracto", type=["csv","txt","xlsx","xls"],
...
with col2:
    ...
    archivo_xlsx = st.file_uploader("Subir Excel", type=["xlsx","xls"],
...
if archivo_csv and archivo_xlsx:
    ...
    df_resultado = comparar(serie_csv, desc_csv, df_excel,
                            col_debito, col_credito, col_tercero, tolerancia)
    ...
    with m1:
        st.markdown(f'<div class="metric-card"><div class="number blue">{total}</div>'
                    f'<div class="label">Valores CSV analizados</div></div>', unsafe_allow_html=True)
    with m2:
        st.markdown(f'<div class="metric-card"><div class="number green">{encontrados}</div>'
                    f'<div class="label">Encontrados en Excel</div></div>', unsafe_allow_html=True)
    with m3:
        st.markdown(f'<div class="metric-card"><div class="number red">{no_encontrados}</div>'
                    f'<div class="label">No encontrados</div></div>', unsafe_allow_html=True)
    with m4:
        color = "green" if pct == 100 else ("orange" if pct >= 80 else "red")
        st.markdown(f'<div class="metric-card"><div class="number {color}">{pct}%</div>'
                    f'<div class="label">Coincidencia</div></div>', unsafe_allow_html=True)
    ...
    # Colorear filas: usar pandas Styler con colores HTML explícitos
    def colorear_fila(row):
        color = "#E8F5E9" if "CORRECTO" in str(row["Estado"]) else "#FFEBEE"
        return [f"background-color: {color}; color: #1E293B"] * len(row)
    ...
    <div style="text-align:center; padding:60px 20px; color:#94A3B8;">
        ...
        <p style="font-size:1.1rem; font-weight:600; color:#64748B;">
            Carga los dos archivos para iniciar la comparativa</p>
